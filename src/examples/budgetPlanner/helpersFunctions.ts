...
import { BPRow, RowCells, RowPair } from "./budgetPlanner";
...
export const getChevronCell = (row: BPRow) =>
  row.cells.find((cell: RowCells) => cell.type === "chevron") as ChevronCell;

const hasChildren = (rows: BPRow[], row: BPRow): boolean =>
  rows.some((r) => getChevronCell(r)?.parentId === row.rowId);
...
export const getExpandedRows = (rows: BPRow[]): BPRow[] =>
  rows.filter((row) => {
    const areAllParentsExpanded = isRowFullyExpanded(rows, row);
    return areAllParentsExpanded !== undefined ? areAllParentsExpanded : true;
  });

export const fillCellMatrixHorizontally = (rows: BPRow[]): BPRow[] =>
  rows.map((row) => {
    const mappedCells = row.cells.map((cell, idx) => ({
    ...
    mappedCells
      ...
      .forEach((mappedCell) => {
        ...
        cells[mappedCell.idx].value =
          cells[mappedCell.idx + 1].value +
          cells[mappedCell.idx + 2].value +
          cells[mappedCell.idx + 3].value;
      });
    ...
    mappedCells
      ...
      .forEach((mappedCell) => {
        ...
        cells[mappedCell.idx].value =
          cells[mappedCell.idx + 1].value +
          cells[mappedCell.idx + 5].value +
          cells[mappedCell.idx + 9].value +
          cells[mappedCell.idx + 13].value;
      });
    return row;
  });
...
export const resetAggregatedMonthFields = (row: BPRow) => {
  row.cells.forEach((cell) => {
    if (
      (cell.type === "number" || cell.type === "nonEditableNumber") &&
...
const collectRowPairsOnChildren = (
  allRows: BPRow[],
  parentRow: BPRow,
  acc: RowPair[]
) => {
...
export const getDirectChildrenRows = (
  rows: BPRow[],
  parentRow: BPRow
): BPRow[] =>
  rows.filter(
...
export const getParentRow = (rows: BPRow[], row: BPRow): BPRow | undefined =>
  rows.find((r) => r.rowId === getChevronCell(row)?.parentId);

const assignIndentAndHasChildrens = (
  allRows: BPRow[],
  parentRow: BPRow,
  indent: number
) => {
...
export const getDataFromRows = (rows: BPRow[]): BPRow[] =>
  rows.filter(
    (row) => row.cells.find((cell) => cell.type === "chevron") !== undefined
  );

export const createIndents = (rows: BPRow[]): BPRow[] =>
  rows.map((row) => {
    const chevronCell = getChevronCell(row);
    if (chevronCell && chevronCell.parentId === undefined) {
      const hasRowChildrens = hasChildren(rows, row);
...
export const isHorizontalChevronCell = (cell: RowCells) =>
  cell.type === "horizontalChevron";

/// COLUMN

export const extendWithColIds = (row: BPRow, columns: BPColumn[]) => {
  row.cells.forEach(
    (cell, idx) => ((cell as any).columnId = columns[idx]?.columnId)
  );
  return row;
};

export const getDataFromColumns = (columns: Column[]): Column[] =>
  columns.slice(1, columns.length);

export const getHorizontalChevronCell = (cells: RowCells[], columnId: Id) =>
  cells.find(
    (cell: RowCells) =>
      cell.type === "horizontalChevron" && cell.parentId === columnId
  ) as HorizontalChevronCell | undefined;

export const getParentCell = (
  cells: RowCells[],
  cell: RowCells
): HorizontalChevronCell | undefined =>
  cells.find((c) => (c as any).columnId === (cell as any).parentId) as
    | HorizontalChevronCell
    | undefined;

export const getDirectChildrenColumns = (
  rows: BPRow[],
  parentRow: BPRow
): BPRow[] =>
  rows.filter(
    (row) =>
      !!row.cells.find(
        (cell) =>
          cell.type === "horizontalChevron" && cell.parentId === parentRow.rowId
      )
  );

export const isCellFullyExpanded = (
  cells: RowCells[],
  cell: RowCells
): boolean => {
...
export const getExpandedCells = (cells: RowCells[]): RowCells[] =>
  cells.filter((cell) => {
    const areAllParentsExpanded = isCellFullyExpanded(cells, cell);
    return areAllParentsExpanded !== undefined ? areAllParentsExpanded : true;
  });

export const getColumnsIdsxToRender = (
  cells: RowCells[],
  columnsToRender: BPColumn[]
) => {
  return cells
    .map((thrCell, idx) =>
      columnsToRender.find(
        (colToRender) =>
          colToRender.columnId === (thrCell as HorizontalChevronCell).columnId
      )
        ? idx
        : NaN
    )
...
export const filterCellsOnRows = (rows: BPRow[], visibleColsIdxs: number[]) =>
  rows.map((row) => {
