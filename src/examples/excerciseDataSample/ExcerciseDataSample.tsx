...
import {
  ...
  initialResults,
  parameters,
} from "../../data/excercisesData/initialValues";
...
export const ExcercisesDataSample: React.FC = () => {
  ...
  const cellChangesHandler = (changes: CellChange[]) => {
    changes.forEach((change) => {
      if (change.columnId === "excercise") {
        setExcercises((excercises) =>
          excercises.map((excercise) =>
            excercise.id === change.rowId
              ? {
                  ...excercise,
                  hidden: !(change.newCell as ChevronCell).isExpanded,
                }
              : excercise
          )
        );
      ...
      setResults((oldResults) => {
        const resultIdx = oldResults.findIndex(
          (oldResult) =>
            oldResult.parameterId === param.id &&
            oldResult.athleteId === athleteId
        );
  ...
  return (
    <div className="excercises-data-sample">
      <ReactGrid
        customCellTemplates={{
